...
class ListItems
{
	...
	/** Load all list items associated with a user ID
	 * This function both outputs <li> tags with list items and 
	 * returns an array with the list ID, list URL, and the order
	 * number for a new item.
	 * @return array - an array containing list ID, list URL and next order
	 */
	public function loadlistItemsByUser()
	{
		$sql = "SELECT list_items.ListID, ListText, ListItemID, ListItemColor, ListItemDone, ListURL
			FROM list_items
			LEFT JOIN lists
			USING (ListID)
	 ...
	 public function changeListItemPosition()
	{
		...
		{ 
			 ...
			 * original position and the position it was moved to.			
			 * If the change makes the item's position greater than
			 * the item's starting position, then the query sets its			 
			 * position to the new position. Otherwise, the position
		...
		else
		{
			 ...
			 * and if the item's changed position is less than the 
			 * starting position, its position is set to the new  
			 * position.
			 */
			$sql = "UPDATE list_items
				SET ListItemPosition=(
	 ...
	 * @return mixed - return TRUE on success, error message on failure
	 */
